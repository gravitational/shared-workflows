...
func TestBloatCheckDirectories(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			test.errAssertion(t, b.BloatCheckDirectories(context.Background(), base, current, []string{"one", "two", "three"}, &out))
		})
	}
}
...
func TestBloatCheck(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			// Validate that only the entries excluded by admins exist in the output
			var out bytes.Buffer
			test.errAssertion(t, b.BloatCheck(context.Background(), "./testdata/base_bloat.json", current, []string{"one", "two", "three"}, &out))
		})
	}
}

func TestSaveBaseStats(t *testing.T) {
	build := t.TempDir()
	b := &Bot{}
	var buffer bytes.Buffer

	// missing files should cause a failure
	require.Error(t, b.SaveBaseStats(context.Background(), build, []string{"one", "two", "four"}, &buffer))
	...
	require.NoError(t, b.SaveBaseStats(context.Background(), build, []string{"one", "two", "four"}, &buffer))
