...
func (b *Bot) CheckDocsPathsForMissingRedirects(ctx context.Context, teleportClonePath string) error {
	...
	// Check if a file on disk corresponds to the route that Docusaurus
	// creates for a renamed or deleted docs page. If one still does, there
	// is no need for a redirect.
	missingPaths := []string{}
	for _, r := range m {
		docspaths := toDocsPaths(r)
		_, err1 := os.Stat(filepath.Join(teleportClonePath, docspaths.standalone))
		_, err2 := os.Stat(filepath.Join(teleportClonePath, docspaths.category))
		if err1 != nil && err2 != nil {
			missingPaths = append(missingPaths, r)
		}
	}

	if len(missingPaths) > 0 {
		return trace.Errorf("docs config at %v is missing redirects for the following renamed or deleted pages: %v", docsConfigPath, strings.Join(missingPaths, ", "))
	}
...
// possibleDocsPaths represents the file paths possible for a given docs page
// URL. For example, /mypage can either refer to docs/pages/mypage.mdx or
// docs/pages/mypage/mypage.mdx.
//
// See:
// https://docusaurus.io/docs/sidebar/autogenerated#category-index-convention
type possibleDocsPaths struct {
	standalone string
	category   string
}

// toDocsPaths returns the file paths possible for a given docs
// page URL. For example, /mypage can either refer to docs/pages/mypage.mdx or
// docs/pages/mypage/mypage.mdx.
//
// See:
// https://docusaurus.io/docs/sidebar/autogenerated#category-index-convention
func toDocsPaths(url string) possibleDocsPaths {
	return possibleDocsPaths{
		standalone: filepath.Join(docsPrefix, strings.TrimRight(url, "/")+".mdx"),
		category:   filepath.Join(docsPrefix, url, path.Base(url)+".mdx"),
	}
}
