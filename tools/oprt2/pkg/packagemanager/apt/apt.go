...
// APT is a [packagemanager.PackageManager] that publishes Debian files to APT repos.
type APT struct {
...
var _ packagemanager.ClosablePackageManager = &APT{}
...
// WithComponents configures the package manager to publish files to APT components. Key is
// the name of each component to publish to, value is a list of regular expressions used to
// match files to the component. Each expression will be matched against every file in the
// storage backend, including the file's path. For example, `some/path/teleport.deb`.
...
func (apt *APT) EnqueueForPublishing(ctx context.Context, queue *errgroup.Group) error {
	logger := logging.FromCtx(ctx)

	// Collect possible files to upload
	candidateItems, err := apt.fm.ListItems(ctx)
	if err != nil {
		return fmt.Errorf("failed to get packages to publish to APT repos: %w", err)
	}
	...
	for component, filePatterns := range apt.components {
		for _, candidateItem := range candidateItems {
			for _, filePattern := range filePatterns {
				...
				logger.DebugContext(ctx, "queuing file for publishing", "file", candidateItem)
				localCandidateFilePath, err := apt.fm.GetLocalFilePath(ctx, candidateItem)
				if err != nil {
					return fmt.Errorf("failed to get local file path to %q via file manager %q: %q", candidateItem, apt.fm.Name(), err)
				}
	...
	if err := apt.attuneRunner.Setup(ctx); err != nil {
		return fmt.Errorf("attune runner setup hook failed: %w", err)
	}
