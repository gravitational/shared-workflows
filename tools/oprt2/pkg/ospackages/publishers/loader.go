...
import (
	...
	"github.com/gravitational/shared-workflows/tools/oprt2/pkg/ospackages/publishers/attune"
...
func FromAPTConfig(ctx context.Context, config config.APTPackagePublisher, logger *slog.Logger) (ospackages.APTPublisher, error) {
	switch {
	case config.Discard != nil:
		return discard.NewDiscardPublisher(), nil
	case config.Attune != nil:
		return attune.FromConfig(ctx, *config.Attune, logger)
	default:
