...
func (r *Reconciler) reconcile(ctx context.Context) error {
	...
	for _, run := range runs {
		if req, ok := runToReqMap[run.WorkflowID]; ok {
			...
			if err := r.reviewHandler.HandleReview(ctx, req); err != nil {
		...
		for _, deployment := range pending {
			err := r.deployReviewEventProcessor.ProcessDeploymentReviewEvent(ctx, githubevents.DeploymentReviewEvent{
			...
			if err != nil {
				r.log.Error("failed to process deployment review event", "error", err, "workflow_run_id", run.WorkflowID, "environment", deployment.Environment)
				continue
			}
		}
	}
