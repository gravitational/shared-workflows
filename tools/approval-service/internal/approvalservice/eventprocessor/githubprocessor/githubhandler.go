...
func New(ctx context.Context, cfg config.GitHubSource, tele teleClient, opts ...Opt) (*Processor, error) {
	...
	return newFromClient(ctx, client, cfg, tele, opts...)
}

func newFromClient(ctx context.Context, client ghClient, cfg config.GitHubSource, tele teleClient, opts ...Opt) (*Processor, error) {
	var p Processor
...
// HandleReview implements accessrequest.ReviewHandler.
...
// It is not used in the current implementation, but it is here for future use.
